      ...
      def results
        if cookies[:fit_my_4x4].present?
          ...
          if params[:id].present?
            ...
            @products = @this_category.leaves.map { |c| c.products.active.includes(:vehicles).references(:vehicles).where('(refinery_ironman_vehicles.id in (?) or (refinery_ironman_vehicles.id is null and refinery_ironman_products.id is not null))', @vehicle_filter.values) }.flatten.paginate(:page => params[:page], :per_page => 15)
            ...
            @products = Refinery::Ironman::Product.active.includes(:vehicles).references(:vehicles).where('(refinery_ironman_vehicles.id in (?) or (refinery_ironman_vehicles.id is null and refinery_ironman_products.id is not null))', @vehicle_filter.values).paginate(:page => params[:page], :per_page => 15)
          end
        end
